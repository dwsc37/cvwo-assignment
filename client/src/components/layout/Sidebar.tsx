...
const Sidebar = () => {
    ...
    const handleOpenFeeds = (event: React.MouseEvent) => setOpenFeeds(!openFeeds);

    const handleOpenModules= (event: React.MouseEvent) => setOpenModules(!openModules);
    ...
    const mappedModules = useMemo(() => {
        return isLoading || error || !modules
            ? <LoadingPage />
            : <List>
                {modules.length === 0 && <ListItem sx={{marginLeft:"20px"}}>No Subscribed Modules</ListItem>}
                {modules.map((module) => (
                    <ListItemButton key={module.ID} component={Link} to={`/module/${module.Code}`} sx={{marginLeft:"20px"}}>

                        <ListItemText primary={module.Code} secondary={module.Name}/>
                    </ListItemButton>
                ))}
            </List>
        ...
        >
            ...
            <List>
                ...
                <Collapse in={openFeeds} timeout="auto" unmountOnExit>
                    <Divider/>
                    <ListItemButton component={Link} to={"/home"} sx={{marginLeft:"20px"}}>
                        <ListItemIcon>
                            <HomeIcon />
                        </ListItemIcon>
                        <ListItemText primary="Home" />
                    </ListItemButton>
                    <ListItemButton component={Link} to={"/all"} sx={{marginLeft:"20px"}}>
                        <ListItemIcon>
                            <AnalyticsIcon />
                        </ListItemIcon>
